...
const Score = () => {
  ...
  return (
    <main className="h-full w-full flex flex-col xl:flex-row z-10">
      ...
      <div className="flex flex-col items-center bg-white p-[clamp(32px,4vw,48px)] mb-[clamp(16px,4vw,24px)] rounded-xl md:rounded-3xl shadow-lg">
          {
            questions.filter((topic: ResponseAPI) => `/score/${topic.title.toLowerCase()}` === location.pathname.toLowerCase()).map((topic: ResponseAPI) => 
              <div className="flex items-center gap-3">
                ...
                <span className="text-[clamp(18px,2.5vw,28px)] font-medium text-blue-900">{topic.title}</span>
              </div>
          )
        }
        <span className="text-[clamp(88px,10vw,144px)] font-bold text-blue-900">{score}</span>
        <p className="text-[clamp(18px,2.5vw,28px)] text-grey-500">out of { count }</p>
      </div>
      <ResetButton />
      </section>
    </main>
